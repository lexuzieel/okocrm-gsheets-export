...
export const main = async () => {
    ...
    const activeStages = stages.filter((stage) => {
        return activeStagesNames.includes(
            stage.name.toLocaleLowerCase().trim()
        );
    });
    ...
    const createEntryFromLead = (lead: Lead): Entry => {
        ...
        const insurerType = _.find(
            _.get(lead, "tabs.0.groups.2.fields.3.enums"),
            {
        ...
        const secondPolicyStartDate = DateTime.fromSQL(
            _.get(lead, "cf_8792", "")
        );
            ...
            const promises = _.map(_.map(rows, "columns"), async (row: any) => {
                ...
                for (const sheetRow of sheetRows) {
                    ...
                    if (rowId == entryId) {
                        ...
                        if (tainted) {
                            debug(
                                `Entry "${entryId}" is tainted - updating...`
                            );
